...
const ARCHITECT_SYSTEM = `你是 OmniStage（万象剧场）**WorldForge·架构师 Agent**（并行轨之一）。
...
const MECHANIST_SYSTEM_PARALLEL = `你是 OmniStage（万象剧场）**WorldForge·机制设计师 Agent**（并行轨之一，与架构师/人物卡设计师**并行**）。
...
const CHARACTER_DESIGNER_SYSTEM_PARALLEL = `你是 OmniStage（万象剧场）**WorldForge·人物卡设计师 Agent**（并行轨之一，与架构师/机制设计师**并行**）。
...
> {
  const system =
    "你是 OmniStage（万象剧场）角色增量合并器。只输出一个 JSON 对象，表示“新增/更新角色片段”，不要输出完整 Canonical。";
  const prompt =
